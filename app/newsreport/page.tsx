...
const parseThaiDate = (dateStr: string): Date => {
  // Example dateStr: "21/05/2569 10:30:00" or "21 พ.ค. 2569 10:30:00"
  // We need to parse the date part only for comparison
  const parts = dateStr.split(" ")[0].split("/"); // "21", "05", "2569"
  if (parts.length === 3) {
    const day = parseInt(parts[0], 10);
    const month = parseInt(parts[1], 10);
    const year = parseInt(parts[2], 10) - 543; // Convert Buddhist year to Gregorian
...
export default function NewsReportPage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        await Promise.all([
          fetchNews(), // This depends on categories, so ensure categories are fetched first or handle dependencies
          fetchSources(), // Independent
  ...
  const [sortColumn, setSortColumn] = useState<"id" | "date" | "title">("id"); // เพิ่ม state สำหรับคอลัมน์ที่ใช้เรียงลำดับ
  ...
  const fetchNews = async () => {
    try {
      ...
      const data = await res.json();
      setNewsRows(data.map((item: any) => { // Explicitly type item as any for raw API response
    ...
    return [...newsRows]
      ...
      .sort((a, b) => {
        ...
        if (sortColumn === "id") {
          ...
          compareResult = dateA.getTime() - dateB.getTime(); // เปรียบเทียบ timestamp
        } else if (sortColumn === "title") {
          compareResult = a.title.localeCompare(b.title, "th"); // เปรียบเทียบตามตัวอักษรไทย
        }

        // ใช้ sortOrder เพื่อกำหนดทิศทาง
        return sortOrder === "asc" ? compareResult : -compareResult;
      });
  }, [newsRows, searchTerm, sortOrder, startDate, endDate]);
  ...
  }, [displayedRows, categories, summaryCategoryLabels]);
  ...
  return (
    ...
    <div className="flex">
                    ...
                    <th
                      onClick={() => {
                        if (sortColumn === "id") {
                          ...
                          setSortColumn("id");
                          setSortOrder("asc"); // Default to ascending for ID
                        }
                      }}
                      className="
        px-4 py-3
      ...
      "
                    >
                      ลำดับ {sortColumn === "id" && (sortOrder === "asc" ? "▲" : "▼")}
                    </th>
                    ...
                    <th
                      onClick={() => {
                        if (sortColumn === "date") {
                          ...
                          setSortColumn("date");
                          setSortOrder("desc"); // Default to newest first for date
                        }
                      }}
                      className="px-4 py-3 text-center align-middle font-medium whitespace-normal cursor-pointer select-none"
                    >
                      วันที่/เวลา
                    ...
                    <th
                      onClick={() => {
                        if (sortColumn === "title") {
                          ...
                          setSortColumn("title");
                          setSortOrder("asc"); // Default to A-Z for title
                        }
                      }}
                      className="px-4 py-3 text-center align-middle font-medium whitespace-normal cursor-pointer select-none"
                    >
